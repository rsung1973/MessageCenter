...
namespace WebHome.Helper.LineEvent
{
    ...
    {
        public const String __SLOGAN = @"感謝您加入虹盛保全好友😊
請回覆【hello】訊息取得設定管理連結";
//            @"謝謝您訊息我們喔 😊
...
//祝您有個美好的一天😊";
        ...
        public async Task HandleTextMessage()
        {
            ...
            {
                replyMessage = new TextMessage($"{Settings.Default.HostUrl}/MessageCenter/InfoCenter/RegisterResident?KeyID={HttpUtility.UrlEncode(lineEvent.Source.UserId.EncryptKey())}");
            }
            //else if (message != String.Empty)
            //{
            //    var item = models.GetTable<UserProfile>().Where(u => u.PID == message || u.UserName == message).FirstOrDefault();
            ...
            //    var binding = item.UserBinding.Where(b => b.LineID == lineEvent.Source.UserId)
            //        .FirstOrDefault();
            ...
            //    replyMessage = new TextMessage("警報接收已設定，謝謝😄");
            //}
            else
            {
